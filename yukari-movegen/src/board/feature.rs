...
pub fn index_threat(
    from_piece: Piece, from_square: Square, to_piece: Piece, to_square: Square, king: Square, friendly: bool, attacking_enemy: bool,
) -> Option<usize> {
    ...
    let pawn_threat = || {
        ...
        let map = PAWN_MAP[attacking_enemy][to_piece]?;
    ...
    let piece_threat = |indexes: &[usize; 65], attacks: &[u64; 64], map: &[[Option<usize>; 6]; 2], offset: usize| {
        let to_piece = to_piece as usize;
        let attacking_enemy = usize::from(attacking_enemy);
        let map = map[attacking_enemy][to_piece]?;
    ...
    Some(768 + (usize::from(!friendly) * MAX_OFFSET) + idx)
}
