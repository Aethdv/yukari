...
impl MoveOrder {
    pub fn classify(
        ...
        last_last_last_m: Option<(Piece, Move)>, last_last_m: Option<(Piece, Move)>, last_m: Option<(Piece, Move)>, m: Move,
    ) -> Self {
        ...
        if let Some((last_piece, last_m)) = last_last_last_m {
...
impl Thread {
    ...
    fn update_history(
        &mut self, ply: usize, last_last_last_m: Option<(Piece, Move)>, last_last_m: Option<(Piece, Move)>, last_m: Option<(Piece, Move)>, m: Move, bonus: i32,
    ) {
    ...
    pub fn search(&mut self, depth: i32, mut alpha: i32, beta: i32, ply: usize, tt: &[TtEntry], excluded_move: Option<Move>) -> i32 {
        ...
        let mut moves = {
            ...
            let last_last_last_m = *self.path.iter().rev().nth(2).unwrap_or(&None);
            moves
                .into_iter()
                .map(|m| (m, MoveOrder::classify(&self.board[ply], tt_move, &self.history, &self.conthist, last_last_last_m, last_last_m, last_m, m)))
            ...
            if score >= beta {
                ...
                if !m.is_capture() {
                    for (m, _) in moves.into_iter().take(movecount) {
                        ...
                        self.update_history(ply, last_last_last_m, last_last_m, last_m, m, -bonus);
                    }
                    self.update_history(ply, last_last_last_m, last_last_m, last_m, *m, bonus);
                }
