...
impl Thread {
    ...
    fn update_history(
        &mut self, ply: usize, last_last_last_m: Option<(Piece, Move)>, last_last_m: Option<(Piece, Move)>,
        last_m: Option<(Piece, Move)>, m: Move, bonus: i32,
    ) {
            ...
            moves
                ...
                .map(|m| {
                    (
                        m,
                        MoveOrder::classify(
                            &self.board[ply], tt_move, &self.history, &self.conthist, last_last_last_m, last_last_m, last_m, m,
                        ),
                    )
                })
